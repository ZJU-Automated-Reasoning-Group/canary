...
std::pair<const llvm::GlobalVariable*, size_t> GlobalPointerAnalysis::processConstantGEP(const llvm::ConstantExpr* cexpr, const DataLayout& dataLayout)
{
	...
	// LLVM 14 compatible way to calculate offset
	// Convert GEP instruction to indexes and compute offset
	SmallVector<Value*, 4> indexOps(indexes.begin(), indexes.end());
	Type* ptrTy = baseVal->getType();
	unsigned offset = 0;
	
	// Calculate offset manually for LLVM 14
	for (unsigned i = 1, e = cexpr->getNumOperands(); i != e; ++i) {
		if (StructType *STy = dyn_cast<StructType>(ptrTy)) {
			unsigned fieldNo = cast<ConstantInt>(cexpr->getOperand(i))->getZExtValue();
			offset += dataLayout.getStructLayout(STy)->getElementOffset(fieldNo);
			ptrTy = STy->getElementType(fieldNo);
		} else {
			ptrTy = ptrTy->getPointerElementType();
			// Handle array types
			if (ArrayType *AT = dyn_cast<ArrayType>(ptrTy)) {
				if (ConstantInt *CI = dyn_cast<ConstantInt>(cexpr->getOperand(i))) {
					int64_t arrayIdx = CI->getSExtValue();
					if (arrayIdx != 0) {
						Type *elementTy = AT->getElementType();
						// For LLVM 14, TypeSize is now returned, not an unsigned
						TypeSize typeSize = dataLayout.getTypeAllocSize(elementTy);
						offset += arrayIdx * typeSize.getFixedSize();
					}
				}
				ptrTy = AT->getElementType();
			}
			// Handle vector types
			else if (VectorType *VT = dyn_cast<VectorType>(ptrTy)) {
				if (ConstantInt *CI = dyn_cast<ConstantInt>(cexpr->getOperand(i))) {
					int64_t vecIdx = CI->getSExtValue();
					if (vecIdx != 0) {
						Type *elementTy = VT->getElementType();
						// For LLVM 14, TypeSize is now returned, not an unsigned
						TypeSize typeSize = dataLayout.getTypeAllocSize(elementTy);
						offset += vecIdx * typeSize.getFixedSize();
					}
				}
		...
		else if (auto ce = dyn_cast<ConstantExpr>(baseVal))
		{
			switch (ce->getOpcode())
			{
				...
				case Instruction::BitCast:
					...
					break;
				default:
					llvm::errs() << "Constant expr not yet handled in global intializer: ";
					ce->print(llvm::errs());
					llvm::errs() << "\n";
